...
QAT_DICTIONARIES = ["UKACD", "YAWL", "ABLE", "Moby", "PDL", "BNC", "Broda", "Union"]
...
def req_qat(pat, dict="YAWL"):
    ...
    if isinstance(dict, str):
        ...
        if dict not in QAT_DICTIONARIES:
            raise ValueError(f"Dictionary '{dict}' is not supported. Choose from {QAT_DICTIONARIES}.")
        dict = QAT_DICTIONARIES.index(dict)
    url = f"https://www.quinapalus.com/cgi-bin/qat?pat={pat}&dict={dict}"
    response = requests.get(url)
    if response.status_code != 200:
        raise ConnectionError(f"Failed to connect to QAT service. Status code: {response.status_code}")
...
def get_words(analyze_result):
    ...
    for length in range(max_length, 0, -1):
        ...
        if length not in results:
            results[length] = []
        pat = f'{length}:*/' + ''.join(letters).replace("*", ".").replace("!", ".").lower()
        try:
            words = req_qat(pat)
            if length in words:
